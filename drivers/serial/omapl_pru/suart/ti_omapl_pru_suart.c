...
static void omapl_pru_rx_chars(struct omapl_pru_suart *soft_uart, u32 uart_no, u16 rx_status, u32 fifo_bytes, u32 config_reg, unsigned char * data_pointer, u16 ctrl_reg)
{
	...
	do {
		...
		CHN_TXRX_STATUS_RDY) == CHN_TXRX_STATUS_RDY) {
			pru_softuart_clrRxStatus(&soft_uart->suart_hdl[port->line]);
		        ...
		        rx_status = pru_softuart_getRxStatus(&soft_uart->suart_hdl[port->line]);
			pru_softuart_clrRxStatus(&soft_uart->suart_hdl[port->line]);
			fifo_bytes = pru_softuart_getRxFifoBytes(&soft_uart->suart_hdl[port->line]);
			config_reg = pru_softuart_getRxConfig2(&soft_uart->suart_hdl[port->line]);
			data_pointer = pru_softuart_getRxDataPointer(&soft_uart->suart_hdl[port->line]);
			ctrl_reg = pru_softuart_getRxCntrlReg(&soft_uart->suart_hdl[port->line]);
			omapl_pru_rx_chars(soft_uart, port->line, rx_status, fifo_bytes, config_reg, data_pointer, ctrl_reg);
		}
	}
				  ...
				  struct ktermios *old)
{
	...
	unsigned char dir_delay_offset = 2;  // includes start and 1-stop bit
...
/* Set parity */
        ...
        if (SUART_SUCCESS !=
            pru_softuart_setparity(&soft_uart->suart_hdl[port->line], cval, cval))
...
/* Set bits per character */
	...
	if (SUART_SUCCESS !=
            pru_softuart_setstopbits(&soft_uart->suart_hdl[port->line], cval_s))
 ...
 */
	...
	/*
	 * Calculate the dir delay offset (bits)
	 */
        if (SUART_SUCCESS !=
            pru_softuart_setdir(&soft_uart->suart_hdl[port->line],
                                 dir_delay_offset, soft_uart->clk_freq_timer2,
                                 SUART_DEFAULT_BAUD / baud))
                __suart_err("failed to set dir \n"); //to: %d\n", baud);
...
static int pru_suart_request_port(struct uart_port *port)
{
	...
	if (soft_uart->suart_hdl[port->line].uartNum == PRU_SUART_UART1) {
		...
		if(PRU_SUART1_CONFIG_DIR_EN == PRU_SUART_DIR)
			pru_suart_config.DIREnable = 1;//PRU_SUART_UART1 + (PRU_SUART_UART1-1)*4;
		else
                        pru_suart_config.DIREnable = PRU_SUART_DIR_DISABLED;
               pru_suart_config.DIRDelay = PRU_SUART1_CONFIG_DIR_DEL;
               pru_suart_config.DIRPin = PRU_SUART1_CONFIG_DIR_PIN;
		...
		if(PRU_SUART2_CONFIG_DIR_EN == PRU_SUART_DIR)
			pru_suart_config.DIREnable = 5;//PRU_SUART_UART2 + (PRU_SUART_UART2-1)*4;
                else
                       pru_suart_config.DIREnable = PRU_SUART_DIR_DISABLED;
	        pru_suart_config.DIRDelay = PRU_SUART2_CONFIG_DIR_DEL;
                pru_suart_config.DIRPin = PRU_SUART2_CONFIG_DIR_PIN;
		...
		if(PRU_SUART3_CONFIG_DIR_EN == PRU_SUART_DIR)
			pru_suart_config.DIREnable = 9;//PRU_SUART_UART3 + (PRU_SUART_UART3-1)*4;
                else
                        pru_suart_config.DIREnable = PRU_SUART_DIR_DISABLED;
                pru_suart_config.DIRDelay = PRU_SUART3_CONFIG_DIR_DEL;
                pru_suart_config.DIRPin = PRU_SUART3_CONFIG_DIR_PIN;
		...
		pru_suart_config.RXSerializer = PRU_SUART4_CONFIG_RX_SER;
                if(PRU_SUART4_CONFIG_DIR_EN == PRU_SUART_DIR)
			pru_suart_config.DIREnable = 13;//PRU_SUART_UART4 + (PRU_SUART_UART4-1)*4;
                else
                        pru_suart_config.DIREnable = PRU_SUART_DIR_DISABLED;
                pru_suart_config.DIRDelay = PRU_SUART4_CONFIG_DIR_DEL;
                pru_suart_config.DIRPin = PRU_SUART4_CONFIG_DIR_PIN;
	...
	if (PRU_SUART_SUCCESS !=
	    pru_softuart_setconfig(&soft_uart->suart_hdl[port->line],
				   &pru_suart_config, soft_uart->clk_freq_timer2)) {
		dev_err(&pdev->dev,
...
static int __devinit omapl_pru_suart_probe(struct platform_device *pdev)
{
	...
	lsd_dbg(LSD_DBG,"enter function omapl_pru_suart_probe\n");
	...
	for (i = 0; i < PLATFORM_SUART_RES_SZ; i++) {
		...
		if (!res_mem[i]) {
			dev_err(&pdev->dev,
				"unable to get pru memory resources!i=%d\n",i);
			err = -ENODEV;
			goto probe_exit;
		}
	}
	...
	if (!request_mem_region(res_mem[4]->start, resource_size(res_mem[4]),
                                dev_name(&pdev->dev))) {
                dev_err(&pdev->dev, "timer2 memory region already claimed!\n");
                err = -EBUSY;
                goto probe_exit_4;
        }
	...
	soft_uart->pru_arm_iomap.timer2_io_addr = ioremap(res_mem[4]->start,
                                                        resource_size(res_mem
                                                                      [4]));
        if (!soft_uart->pru_arm_iomap.timer2_io_addr) {
                dev_err(&pdev->dev, "ioremap failed\n");
                err = -ENOMEM;
	...
	if (PRU_SUART1_CONFIG_DIR_EN | PRU_SUART2_CONFIG_DIR_EN | PRU_SUART3_CONFIG_DIR_EN | PRU_SUART4_CONFIG_DIR_EN){
                soft_uart->clk_timer2 = clk_get(NULL, "timer2_pru");
                if (IS_ERR(soft_uart->clk_timer2)) {
                        dev_err(&pdev->dev, "no clock available: timer2\n");
                        err = PTR_ERR(soft_uart->clk_timer2);
                        soft_uart->clk_timer2 = NULL;
                        goto probe_exit_clk_timer2;
                }
                soft_uart->clk_freq_timer2 = clk_get_rate(soft_uart->clk_timer2);
                clk_enable(soft_uart->clk_timer2);
        }
	...
	dma_phys_addr = res_mem[5]->start;
        dma_vaddr_buff = ioremap(res_mem[5]->start, resource_size(res_mem[5]));
	if (!dma_vaddr_buff) {
	...
	if (PRU_SUART1_CONFIG_DIR_EN | PRU_SUART2_CONFIG_DIR_EN | PRU_SUART3_CONFIG_DIR_EN | PRU_SUART4_CONFIG_DIR_EN){
               dev_info(&pdev->dev,
                        "%s device registered"
                        "(pru_clk=%d, asp_clk=%d, timer2_clk=%d)\n",
                        DRV_NAME, soft_uart->clk_freq_pru, soft_uart->clk_freq_mcasp, soft_uart->clk_freq_timer2);
       }
       else {
		dev_info(&pdev->dev,
			 "%s device registered"
...
probe_exit_clk_timer2:
        clk_put(soft_uart->clk_timer2);
probe_exit_clk_pru:
...
probe_exit_free_region:
	release_mem_region(res_mem[4]->start, resource_size(res_mem[4]));
probe_exit_4:
...
static int __devexit omapl_pru_suart_remove(struct platform_device *pdev)
{
	...
	clk_disable(soft_uart->clk_timer2);
	pru_softuart_deinit();
	...
	iounmap(soft_uart->pru_arm_iomap.psc0_io_addr);
	iounmap(soft_uart->pru_arm_iomap.psc1_io_addr);
	iounmap(soft_uart->pru_arm_iomap.timer2_io_addr);
	release_mem_region(res_mem[0]->start, resource_size(res_mem[0]));
	release_mem_region(res_mem[1]->start, resource_size(res_mem[1]));
	release_mem_region(res_mem[2]->start, resource_size(res_mem[2]));
	release_mem_region(res_mem[3]->start, resource_size(res_mem[3]));
...
{
	int ret;
	lsd_dbg(LSD_DBG,"enter function pru_suart_init\n");
